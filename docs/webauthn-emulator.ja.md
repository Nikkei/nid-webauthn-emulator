...
4. **WebAuthn PRF 拡張**: `create`、`get`、`createJSON`、`getJSON`は、`hmacSecret`を有効にした Authenticator と連携して PRF リクエストを処理します。

5. **Authenticator 情報**: `getAuthenticatorInfo`メソッドで Authenticator の詳細情報を取得できます。

## PRF 拡張

`create`、`createJSON`、`get`、`getJSON`は、WebAuthn の [PRF 拡張](https://www.w3.org/TR/webauthn-3/#prf-extension) をサポートします。利用するには、`hmacSecret` を有効にした `AuthenticatorEmulator` が必要です（[authenticator emulator ドキュメント](authenticator-emulator.ja.md)を参照）。

入力は `extensions.prf` で指定し、出力は `getClientExtensionResults().prf` から取得します。
...
const credential = emulator.create(origin, {
  publicKey: {
    ...creationOptions,
    // first（および任意で second）は BufferSource の入力です
    extensions: { prf: { eval: { first: salt1, second: salt2 } } },
  },
});

// prf.results は 32 バイトの ArrayBuffer で、同じ salt と資格情報に対しては常に同じ値になります
const {first, second} = credential.getClientExtensionResults().prf!.results;
```

`AuthenticatorEmulator` に渡す `hmacSecret` オプションによって、`getClientExtensionResults().prf` に鍵が含まれるタイミングが変わります。

- **`"none"`**: （デフォルト）PRF 拡張は無効化され、鍵は返されません。
- **`"hmac-secret"`**: `create` は `prf.enabled` を `true` にするだけで `prf.results` は返さず、`get` が `prf.results` に鍵を返します。
- **`"hmac-secret-mc"`**: `create`・`get` のいずれも `prf.enabled` を `true` にし、`prf.results` に鍵を返します。

JSON 版の `createJSON` と `getJSON` は、上記と同じ入力を base64url 文字列で受け取り、`prf.results` も base64url 文字列で返します。
...
3. このエミュレータは、PRF 拡張（[PRF 拡張](#prf-拡張)を参照）を含む WebAuthn 仕様の基本的な機能をカバーしていますが、すべての高度な機能や拡張機能をサポートしているわけではありません。
